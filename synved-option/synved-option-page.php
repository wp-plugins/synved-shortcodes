...
function synved_option_page_link_url($id, $name, $item = null)
{
	...
	if ($type == 'options-page')
	{
		...
		{
			return $parent . '?page=' . $synved_option_list[$id]['pages'][$name]['wp-page-slug'];
		}
	}
...
?>
	<div class="wrap">
		...
		<?php synved_option_render_page($name); ?>
		<p class="submit">
...
function synved_option_page_add($id, $name, $item)
{
	...
	if ($type == 'options-page')
	{
		...
		$page = $addfunc($parent, $tip, $label, $role, $page_slug, create_function('', 'return synved_option_page_cb(\'' . $id . '\', \'' . $name . '\', synved_option_item_find(\'' . $id . '\', \'' . $name . '\'));'));
